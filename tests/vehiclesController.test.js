...
// Installed supertest npm install supertest jest
const request = require('supertest');
const express = require ('express');
const { getvehicles } = require('../controllers/vehiclesControllers.js');

const app = express();
app.get('/api/vehicles', getvehicles);

describe('GET /api/vehicles', () => {
  test('should return an error if car value is missing', async () => {
        const response = await request(app).get('/api/vehicles');
        expect(response.status).toBe(200);
    });

    test('should return an error if car value is not a valid number', async () => {
        const response = await request(app).get('/api/vehicles?Model=invalid');
        expect(response.status).toBe(200);
    });
    test('should correctly transform the model string to a sum of character values', () => {
      const testCases = [
